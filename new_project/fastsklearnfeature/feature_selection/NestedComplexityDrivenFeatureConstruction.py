...
from fastsklearnfeature.feature_selection.evaluation.NestedCachedEvaluationFramework import NestedCachedEvaluationFramework
...
class ComplexityDrivenFeatureConstruction(NestedCachedEvaluationFramework):
    def __init__(self, dataset_config, classifier=LogisticRegression, grid_search_parameters={'penalty': ['l2'],
        ...
        self.name_to_validation_transformed = {}
        self.name_to_train_and_validation_transformed = {}
    ...
    def materialize_raw_features(self, candidate):
        ...
        temp_fold = [None] * self.folds

        for fold in range(len(self.preprocessed_folds)):
            temp_fold[fold] = self.preprocessed_folds[fold][1] #saves indices

        q = deque(range(self.folds))

        store_cv_sets_train = {}
        self.store_cv_sets_train_target = {} #TODO: the target are candidate independent create in the beginning

        store_cv_sets_validation = {}
        self.store_cv_sets_validation_target = {} #TODO replace with target from test

        store_cv_sets_train_and_validation = [None] * self.folds
        self.store_cv_sets_train_and_validation_target = [None] * self.folds

        store_cv_sets_test = [None] * self.folds
        self.store_cv_sets_test_target = [None] * self.folds #TODO: the target are candidate independent create in the beginning


        for outer_i in range(self.folds):
            test_fold = q.pop()
            store_cv_sets_test[test_fold] = raw_feature[temp_fold[test_fold]]
            self.store_cv_sets_test_target[test_fold] = self.current_target[temp_fold[test_fold]]

            all_indices = []
            for t_f in q:
                all_indices.extend(temp_fold[t_f])
            store_cv_sets_train_and_validation[test_fold] = raw_feature[all_indices]
            self.store_cv_sets_train_and_validation_target[test_fold] = self.current_target[all_indices]
            ...
            for inner_i in range(self.folds - 1):
                ...
                all_indices = []
                for t_f in training_folds:
                    all_indices.extend(temp_fold[t_f])
                store_cv_sets_train[frozenset(training_folds)] = raw_feature[all_indices]
                self.store_cv_sets_train_target[frozenset(training_folds)] = self.current_target[all_indices]
                store_cv_sets_validation[frozenset(training_folds)] = raw_feature[temp_fold[validation_fold]]
                self.store_cv_sets_validation_target[frozenset(training_folds)] = self.current_target[temp_fold[validation_fold]]
        ...
        self.name_to_train_transformed[str(candidate)] = store_cv_sets_train
        self.name_to_validation_transformed[str(candidate)] = store_cv_sets_validation
        self.name_to_train_and_validation_transformed[str(candidate)] = store_cv_sets_train_and_validation
        self.name_to_test_transformed[str(candidate)] = store_cv_sets_test
        ...
        for c in range(1, limit_runs):
            ...
            if c == 1:
                ...
                for raw_f in self.raw_features:
                    ...
                    raw_f.derive_properties(self.name_to_train_and_validation_transformed[str(raw_f)][0])
            ...
            for result in results:
                ...
                if accuracy_delta / self.complexity_delta > self.epsilon:
                    ...
                    self.name_to_validation_transformed[str(candidate)] = result['validation_transformed']
                    self.name_to_train_and_validation_transformed[str(candidate)] = result['train_and_validation_transformed']
