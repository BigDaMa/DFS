...
class Run_RawFeatures(EvaluationFramework):
    ...
    def run(self):
        ...
        features = pickle.load(open('/tmp/all_fds.p', "rb"))[self.which_fd]
...
if __name__ == '__main__':
    ...
    #for n in range(len(pickle.load(open('/tmp/all_fds.p', "rb")))):
    for n in range(20):
        all_results: List[CandidateFeature] = []
        for rotation in range(10):
            selector = Run_RawFeatures(dataset, reader=OnlineOpenMLReader(task_id, 1, rotation), score=make_scorer(roc_auc_score), max_complexity=3, which_fd=n) #make_scorer(f1_score, average='micro') #make_scorer(roc_auc_score)
        ...
        avg_test_accuracy = np.mean([c.runtime_properties['test_score'] for c in all_results])
        print("Average test score: " + str(avg_test_accuracy))

        fd_accuracy.append(avg_test_accuracy)
