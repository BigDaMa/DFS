...
def f_to_min1(hps, X, y, ncv=5):
	...
	if np.sum(mask)	> max_complexity:
		print('result: ' + str(np.sum(mask) - max_complexity))
		return {'loss': np.sum(mask) - max_complexity, 'status': STATUS_OK, 'model': None}
	if np.sum(mask)	== 0:
...
first_run = {}
for f_i in range(X_train.shape[1]):
	if max_complexity-1 < f_i:
		first_run['f_' + str(f_i)] = 0
	else:
		first_run['f_' + str(f_i)] = 1

trials = generate_trials_to_calculate([first_run])
'''
i = 1
while True:
