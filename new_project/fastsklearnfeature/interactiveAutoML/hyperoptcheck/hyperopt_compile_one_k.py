...
def f_clf1(hps):
	...
	model = Pipeline([
		('scale', MinMaxScaler()),
		('selection', wrap.WrapperBestK(SelectKBest(score_func=score_functions[hps['ranker']['type']], k=hps['k']+1))),
...
list_of_rankers = []
for k, v in score_functions.items():
	list_of_rankers.append({'type': k})

space = {'k': hp.randint('ranker_k', max_complexity), 'ranker': hp.choice('selection_type', list_of_rankers)}
