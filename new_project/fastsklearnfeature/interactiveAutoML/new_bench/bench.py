...
fe = ComplexityDrivenFeatureConstruction(None, reader=ScikitReader(X_train, y_train,feature_names=['V' + str(i) for i in range(500)]),
...
my_pipeline = Pipeline([('features', all_standardized.pipeline),
						...
						('new_construction', ConstructionTransformer(c_max=3, scoring=auc, n_jobs=4, model=LogisticRegression(), cv=5)), #helps to uncover non-linear relationships
						('selection', SelectFromModel(DecisionTreeClassifier())),
