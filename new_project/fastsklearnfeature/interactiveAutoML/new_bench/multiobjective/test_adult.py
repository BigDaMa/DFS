...
def f_clf1(hps):
	...
	model = Pipeline([
		('selection', WeightedRankingSelection(scores=rankings, weights=weights, k=hps['k'] + 1)),
		('clf', LogisticRegression())
	])
...
def f_to_min1(hps, X, y, ncv=5):
	...
	loss = 0.0
	if cv_acc >= min_accuracy and cv_fair >= min_fairness:
	...
	print("fair: " + str(cv_fair) + "acc: " + str(cv_acc) + ' => loss: ' + str(loss))

	return {'loss': loss, 'status': STATUS_OK, 'model': model}



space = {'k': hp.randint('k', X_train.shape[1]),
...
while True:
	...
	if trials.best_trial['result']['loss'] < 0.0:
		break
