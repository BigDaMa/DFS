...
f1_scorer = make_scorer(f1_score, greater_is_better=True)
inner_cv = GroupKFold(n_splits=10).split(X_data, None, groups=groups)
...
rf_random.fit(X_data[:, [0,1,3,4,5,6]], y_data)
...
importances =  {}
for name_i in range(len(names_features)):
	importances[names_features[name_i]] = rf_random.best_estimator_.feature_importances_[name_i]
