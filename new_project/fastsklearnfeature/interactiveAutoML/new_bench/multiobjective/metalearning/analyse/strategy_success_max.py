...
for run in range(len(dataset['best_strategy'])):
	...
	if best_strategy == 0:
		for s in range(1, len(mappnames) + 1):
			if s in dataset['success_value'][run] and len(dataset['success_value'][run][s]) > 0 and dataset['success_value'][run][s][0] == False and s in dataset['times_value'][run]:
				print(mappnames_new[s])
				terminated_without_success[s].append(True)#terminated without success
			else:
				terminated_without_success[s].append(False)#killed without success


for s in range(1, len(mappnames) + 1):
	print(mappnames_new[s] + " finished without success: " + str(np.sum(terminated_without_success[s]) / float(len(terminated_without_success[s]))))