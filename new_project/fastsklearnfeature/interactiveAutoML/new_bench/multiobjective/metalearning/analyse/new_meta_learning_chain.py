...
def get_best_for_fold_predictions(test_ids):
	all_success = []
	for p_i in range(len(test_ids)):
		...
		all_success.append(best_strategy)
...
for train_ids, test_ids in outer_cv_all:
	predictions_probabilities = np.zeros((len(success_ids), len(mappnames)))

	f1_scorer = make_scorer(f1_score, greater_is_better=True)
	for my_strategy in range(strategy_success.shape[1]):

		inner_cv = GroupKFold(n_splits=2).split(X_data[train_ids, :], None, groups=groups[train_ids])

		ttrain_ids, tval_ids = list(inner_cv)[0]
		'''
		...
		rf_random.fit(X_data[train_ids[ttrain_ids]][:, my_ids], strategy_success[train_ids[ttrain_ids], my_strategy])
		...
		predictions_probabilities[:, my_strategy] = rf_random.predict_proba(X_data[:, my_ids])[:,1]
	...
	rf_random = RandomForestClassifier(n_estimators=1000, class_weight='balanced')
	new_dataset_train = np.hstack([X_data[train_ids[tval_ids]][:, my_ids], predictions_probabilities[train_ids[tval_ids]]])
	new_dataset_test = np.hstack([X_data[test_ids][:, my_ids], predictions_probabilities[test_ids]])

	rf_random.fit(new_dataset_train, get_best_for_fold_predictions(train_ids[tval_ids]))

	predictions = rf_random.predict(new_dataset_test)
