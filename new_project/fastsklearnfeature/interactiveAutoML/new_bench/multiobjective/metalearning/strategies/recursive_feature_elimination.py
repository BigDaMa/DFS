...
def recursive_feature_elimination(X_train, X_test, y_train, y_test, names, sensitive_ids, ranking_functions= [], clf=None, min_accuracy = 0.0, min_fairness=0.0, min_robustness=0.0, max_number_features=None, max_search_time=np.inf, cv_splitter=None, floating=True):
	...
	def f_to_min1(hps):
		...
		return {'loss': loss, 'status': STATUS_OK, 'model': model, 'cv_fair': cv_fair, 'cv_acc': cv_acc, 'cv_robust': cv_robust, 'cv_number_features': cv_number_features, 'updated_parameters': hps, 'best_estimator': cv.best_estimator_}
	...
	while len(current_feature_set) >= 1:
		...
		combo_res, combo_result = execute_feature_combo(feature_combo, number_of_evaluations)
		combo_loss = combo_res['loss']
		print('loss: ' + str(combo_loss))
		if len(combo_result) > 0:
			return combo_result

		worst_id = np.argmin(_get_feature_importances(combo_res['best_estimator'].named_steps['clf']))
