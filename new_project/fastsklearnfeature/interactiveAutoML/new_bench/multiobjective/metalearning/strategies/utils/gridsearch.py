...
def run_grid_search(pipeline, X_train, y_train, X_validation, y_validation, accuracy_scorer, sensitive_ids, min_fairness, min_accuracy, min_robustness, max_number_features, model_hyperparameters, start_time, avoid_robustness=False):
    ...
    for configuration in search_configs:
        ...
        print(new_pipeline)
        new_pipeline.named_steps['selection'].fit(X_train, y_train)
...
def is_utility_defined(min_fairness, min_accuracy, min_robustness, max_number_features):
    return min_fairness == np.inf or min_accuracy == np.inf or min_robustness == np.inf or max_number_features == -np.inf