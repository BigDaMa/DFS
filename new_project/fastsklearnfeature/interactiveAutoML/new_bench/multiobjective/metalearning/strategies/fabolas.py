...
def run_fabolas(X_train, X_test, y_train, y_test, names, sensitive_ids, ranking_functions= [], clf=None, min_accuracy = 0.0, min_fairness = 0.0, min_robustness = 0.0, max_number_features = None, max_search_time=np.inf, cv_splitter = None):

	X_train_fab, X_val, y_train_fab, y_val = train_test_split(X_train, y_train, test_size=0.2, random_state=42, stratify=y_train)
	...
	fair_val = make_scorer(true_positive_rate_score, greater_is_better=True, sensitive_data=X_val[:, sensitive_ids[0]])
	...
	def f_to_min1(x, s):
		...
		cv_robust = 1.0 - robust_score_test(X_test=X_val, y_test=y_val, model=model.named_steps['clf'], feature_selector=model.named_steps['selection'], scorer=auc_scorer)
		...
		print("accuracy: " + str(cv_acc) + ' fair: ' + str(cv_fair) + ' k: ' + str(cv_number_features))
