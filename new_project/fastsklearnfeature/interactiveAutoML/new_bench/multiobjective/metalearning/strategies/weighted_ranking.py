...
def weighted_ranking(X_train, X_test, y_train, y_test, names, sensitive_ids, ranking_functions= [], clf=None, min_accuracy = 0.0, min_fairness = 0.0, min_robustness = 0.0, max_number_features = None, max_search_time=np.inf, cv_splitter = None):
	...
	def f_to_min1(hps):
		...
		return {'loss': loss, 'status': STATUS_OK, 'model': model, 'cv_fair': cv_fair, 'cv_acc': cv_acc, 'cv_robust': cv_robust}
	...
	while True:
		...
		if cv_fair >= min_fairness and cv_acc >= min_accuracy and cv_robust >= min_robustness:
