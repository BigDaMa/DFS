...
from fastsklearnfeature.interactiveAutoML.new_bench.multiobjective.metalearning.openml_data.private_models.randomforest.sam_node import node
...
class DP_Random_Forest:
    ...
    def __init__(self,
        ...
        for i in range(self._num_trees):
            self._trees.append(self.build_tree(attribute_indexes, train[i * subset_size:(i + 1) * subset_size], epsilon, self.class_labels))
    ...
    def predict(self, test):
        ...
        for i in range(self._num_trees):
            curr_votes = self.tree_predict(self._trees[i], test)
    ...
    def tree_predict(self, tree, test):
        voted_labels = [defaultdict(int) for x in test]
        for i, rec in enumerate(test):
    ...
    def make_children(self, candidate_atts, parent_node, current_depth, splitting_value_from_parent, svfp_numer,
        ...
        if not candidate_atts or current_depth >= self._max_depth + 1:  # termination conditions. leaf nodes don't count to the depth.
            ...
            current_node = node(parent_node, splitting_value_from_parent, new_splitting_attr, current_depth,
                                     self._id, [], svfp_numer=svfp_numer)  # make a new node
