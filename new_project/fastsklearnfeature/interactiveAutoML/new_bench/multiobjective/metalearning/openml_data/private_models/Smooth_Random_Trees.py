...
import numpy as np
import math
from fastsklearnfeature.interactiveAutoML.new_bench.multiobjective.metalearning.openml_data.private_models.node import node
...
class DP_Random_Forest:    
    ...
    def __init__(self, 
        ...
        for i in range(self._num_trees):
            self.build_tree(attribute_indexes, train[i*subset_size:(i+1)*subset_size], epsilon, class_labels)
    ...
    def build_tree(self, attribute_indexes, train, epsilon, class_labels):
        ...
        self.tree = Tree(attribute_indexes, root, self)
        self.tree.filter_training_data_and_count(train, epsilon, class_labels)

    def predict(self, X):
        predictions = []
        for rec in X:
            predictions.append(self.tree.classify(self.tree._root_node, rec))
        return predictions
