...
for min_accuracy in [0.65, 0.68, 0.71, 0.74, 0.77]:
	for max_number_features in [0.1, 0.09, 0.08, 0.07, 0.06, 0.05, 0.04, 0.03, 0.02, 0.01]:
		...
		with open('/tmp/current_heat_map_complexity_acc.txt', 'w+') as f:
			f.write(str(results_heatmap) + ' current position: features: ' + str(max_number_features) + ' acc: ' + str(
				min_accuracy))
		...
		if np.sum(success_per_strategy) == 0:
			...
			results_heatmap[(min_accuracy, max_number_features)] = (time_per_strategy[fastest_strategy_id] / float(success_per_strategy[fastest_strategy_id]), fastest_strategy_id)
