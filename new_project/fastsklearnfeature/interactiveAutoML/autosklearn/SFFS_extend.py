...
def f2str(feature_combo, featurenames, base_featurenames):
	my_str = 'base features, '
	for f in feature_combo:
		if not featurenames[f] in base_featurenames:
			my_str += featurenames[f] + ', '
	my_str = my_str[:-2]
	return my_str

def parallel_extend(X_train, y_train, X_test=None, y_test=None, floating=True, max_number_features=10, feature_generator=None, folds=3, number_cvs=1):
	...
	for myfi in range(len(feature_generator.numeric_features)):
		...
		if isinstance(feature_generator.numeric_features[myfi], RawFeature) or isinstance(feature_generator.numeric_features[myfi].transformation, OneHotTransformation):
	...
	_, score, my_param = execute_feature_combo1(list(range(len(feature_generator.numeric_features))), feature_combo_id=0,
												params=[0.001, 0.01, 0.1, 1, 10, 100, 1000])
	test_auc = get_test_auc(list(range(len(feature_generator.numeric_features))), X_train_transformed, y_train, X_test_transformed, y_test, my_param)
	...
	print('base features' + ' cv auc: ' + str(
	...
	while (mycount <= max_number_features):
		...
		print(f2str(current_feature_set, featurenames, base_featurenames) + ' cv auc: ' + str(history[frozenset(current_feature_set)]) + ' test auc: ' + str(test_auc))
