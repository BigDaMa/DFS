...
model = xgb.XGBClassifier(objective="binary:logistic", n_estimators=1000, random_state=42)
...
gain_dict = my_pipeline.named_steps['c'].get_booster().get_score(importance_type='gain')

gains = np.zeros(len(numeric_representations))
for r in range(len(numeric_representations)):
	if 'f'+str(r) in gain_dict:
		gains[r] = gain_dict['f'+str(r)]

sorted_ids = np.argsort(gains *-1)
print(gains[sorted_ids])
...
for c in range(1,len(numeric_representations)+1):
	...
	if np.sum(features) > 0:
		...
		pickle.dump(history, open("/tmp/tree_feature_selection.p", "wb"))
