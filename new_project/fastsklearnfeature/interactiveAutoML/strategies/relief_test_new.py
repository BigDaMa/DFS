...
my_pipeline = Pipeline([('f', all_standardized.pipeline),
						('c', ReliefF(n_features_to_select=100, n_neighbors=100)) #SURF(n_features_to_select=100))
						])

my_pipeline.fit(X_train, y_train.values)


print(my_pipeline.named_steps['c'].top_features_)

print(my_pipeline.named_steps['c'].feature_importances_[my_pipeline.named_steps['c'].top_features_])

for c in range(1, len(numeric_representations)+1):
	features = np.zeros(len(numeric_representations))

	features[my_pipeline.named_steps['c'].top_features_[0:c]] = True
...
pickle.dump(history, open("/tmp/relief_feature_selection.p", "wb"))