...
def calculate_prediction_distance(candidate0: CandidateFeature, candidate1: CandidateFeature, name2result_predictions):
    prediction0 = name2result_predictions[str(candidate0)]['probability_estimations_test'][:, 0]
    prediction1 = name2result_predictions[str(candidate1)]['probability_estimations_test'][:, 0]
...
def calculate_prediction_jaccard(candidate0: CandidateFeature, candidate1: CandidateFeature, name2result_predictions):
    prediction0 = name2result_predictions[str(candidate0)]['probability_estimations_test'][:, 0] > 0.5
    prediction1 = name2result_predictions[str(candidate1)]['probability_estimations_test'][:, 0] > 0.5
...
for round in range(len(all_data)):
    ...
    for i in range(len(remaining)):
        if len(remaining[i]['candidate']) == 2:
            ...
            if not isinstance(c, RawFeature) and remaining[i]['score'] >= 0:
                if len(c.parents) == 1:
                    ...
                    #stddev_means = calculate_prediction_distance(remaining[i]['candidate'][0], remaining[i]['candidate'][1], name2result_predictions)
                    stddev_means = calculate_prediction_jaccard(remaining[i]['candidate'][0], remaining[i]['candidate'][1], name2result_predictions)


                    c: CandidateFeature = remaining[i]['candidate'][0]

                    if not 'combo' in operator_stddev_list:
                        operator_stddev_list['combo'] = []
                        operator_gain_list['combo'] = []
                    ...
                    if remaining[i]['score'] >= 0 and max_parent >= 0:
                        operator_gain_list['combo'].append(remaining[i]['score'] - max_parent)
                        operator_stddev_list['combo'].append(stddev_means)
                        print(str('combo') + " stdddev: " + str(stddev_means) + " gain: " + str(remaining[i]['score'] - max_parent))
