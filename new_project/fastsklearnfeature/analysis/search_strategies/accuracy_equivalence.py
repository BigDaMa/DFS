...
for c in string2candidate.values():
	if 'score' in c.runtime_properties:
		if not c.runtime_properties['score'] in prediction2names:
			prediction2names[c.runtime_properties['score']] = []
		prediction2names[c.runtime_properties['score']].append(str(c))
		counter += 1

unnecessary = 0
for name_lists in prediction2names.values():
	if len(name_lists) > 1:
