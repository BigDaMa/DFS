...
for c in string2candidate.values():
	if 'test_fold_predictions' in c.runtime_properties:
		...
		#print(materialized)
		counter += 1
		if not materialized in predictions2names:
			predictions2names[materialized] = []
		predictions2names[materialized].append(str(c))

unnecessary = 0
for name_lists in predictions2names.values():
	if len(name_lists) > 1:
