...
# show best 100 features
def get_max_parent(candidate: CandidateFeature, start=True):
	if start:
		all_max = []
	else:
		all_max = [candidate.runtime_properties['score']]
	for p in candidate.parents:
		all_max.append(get_max_parent(p, False))
	return max(all_max)


print('max score: ' + str(  max([c.runtime_properties['score'] for c in string2candidate.values()])))
...
for c in string2candidate.values():
	if isinstance(c, RawFeature):
		...
		if get_max_parent(c) < c.runtime_properties['score']:
			c.runtime_properties['diff'] = c.runtime_properties['score'] - get_max_parent(c)
			if c.runtime_properties['diff'] > 0.01:
				candidates.append(c)
