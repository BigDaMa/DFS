...
total_search_time = 60*60#60*60#10 * 60
...
while True:
    ...
    _, current_search = run_AutoML(study_uncertainty.best_trial)

    X_train, X_test, y_train, y_test, categorical_indicator, attribute_names = get_data(study_uncertainty.best_trial.params['dataset_id'],
                                                                                        randomstate=study_uncertainty.best_trial.user_attrs['data_random_seed'])

    #get all best validation scores and evaluate them

    original_length = X_meta.shape[0]
    print('oo: ' + str(original_length))
    for my_trial in current_search.study.trials:
        if 'time_since_start' in my_trial.user_attrs and 'pipeline' in my_trial.user_attrs and my_trial.value >= 0.0:
            ...
            X_meta = np.vstack((X_meta, new_features))
            y_meta.append(test_score)

    if original_length == X_meta.shape[0]:
        X_meta = np.vstack((X_meta, copy.deepcopy(study_uncertainty.best_trial.user_attrs['features'])))
        y_meta.append(0.0)
