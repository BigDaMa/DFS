...
data = openml.datasets.get_dataset(dataset_id=1134)

clrs = sns.color_palette("tab20", n_colors=2)
fig = plt.figure()
ax = fig.add_subplot(111)
for i in range(len(my_openml_datasets)):
    data = openml.datasets.get_dataset(dataset_id=str(my_openml_datasets[i]))
    if my_openml_datasets[i] in test_holdout_dataset_ids:
        ax.scatter([data.qualities['NumberOfInstances']], [data.qualities['NumberOfFeatures']], color='red')
    else:
        ax.scatter([data.qualities['NumberOfInstances']], [data.qualities['NumberOfFeatures']], color='blue')
