...
class ExploreKitSelection:
    ...
    def evaluate_canidates(self, candidates):
    ...
    def evaluate_single_candidate(self, candidate):
        new_score = -1.0
        try:
            new_feature_column = candidate.materialize()['train'].reshape(-1, 1)
            new_feature_matrix = new_feature_column
            new_score = self.evaluate(new_feature_matrix)
            print("feature: " + str(candidate) + " -> " + str(new_scores[-1]))
        except:
            pass
        return new_score
    ...
    def run(self):
        ...
        ranked_selected_candidate_ids = self.select_interpretable(35607)#heart
        #ranked_selected_candidate_ids = self.select_interpretable(13264)#diabetes

        start_time = time.time()

        new_scores = self.evaluate_canidates(np.array(self.candidates)[ranked_selected_candidate_ids])
