...
class InitialTraining(): #n_records_for_each_class

    def __init__(self, x, y, n=2, random_seed=42):
        self.x = x
        self.y = y
        self.n = n
    ...
    def generate(self):
        ...
        for y_i in range(self.y.shape[0]):
            ...
            if records_per_class[class_2_id[class_label]] < self.n:
                training_ids.append([ids[y_i]])
                records_per_class[class_2_id[class_label]] += 1

            if np.sum(records_per_class) >= self.n * len(classes):
                break
